...
class Categoria extends JwtController {
		...
		if ($this->consultarPermisosAdmin()) {
			if (isset ( $_REQUEST ["nombre"] )) {
				if (! empty ( $_REQUEST ["nombre"] )) {
					...
					$status = $this->adaptador_model->insert ( "categoria", $campos, array (
							"nombre" 
					) );
					...
					if ($status) {
						...
						echo json_encode ( array (
								"status" => "error",
								"msg" => "Error al insertar categoría nueva, nombre repetido" 
						) );
					}
				} else {
					echo json_encode ( array (
		...
		if ($this->consultarPermisosAdmin()) {
			if (isset ( $_REQUEST ["nombre"] )) {
				if (! empty ( $_REQUEST ["nombre"] ) && ! empty ( $_REQUEST ["id"] )) {
					...
					$status = $this->adaptador_model->update ( "categoria", $id, $campos, array (
							"nombre" 
					) );
