...
app.controller('inscripcionCtrl', function($scope, $http, $window) {
	$scope.filtrado = "";
	$scope.competicion = {
		"id" : 0,
		"nombre" : "",
	...
	$scope.cargar = function() {
		$http.get(base_url + "/inscripcion/listar").then(
				function(response) {
					...
					for (x in response.data["data"]) {
						$scope.inscripciones.push(response.data["data"][x]);
					}
	...
	$scope.modificar = function() {
		...
		config = {
			method : "POST",
			url : base_url + "/inscripcion/modificar",
			params : {
				nombre : $scope.nombreE,
				fecha : $scope.fechaE,
				id : $scope.idE
	...
	$scope.borrar = function(inscripcion) {
