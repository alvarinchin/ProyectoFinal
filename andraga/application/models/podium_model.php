...
class podium_model {
	 ...
	 * @return boolean
	 */
	public function insert($categoria, $especialidad) {
		$podium = R::dispense ( "podium" );
		
		
		$podium->especialidad = $especialidad;
		$podium->categoria = $categoria;
		
		
		
		
		// return $campos;
		if (! $this->existe ( "categoria", "podium", "", $podium)) {
			R::store ( $podium);
			return true;
		} else {
			return false;
